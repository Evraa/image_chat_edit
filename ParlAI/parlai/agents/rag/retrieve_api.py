...
class SearchEngineRetriever(RetrieverAPI):
    ...
    def _query_search_server(self, query_term, n):
        ...
        server_response = requests.post(server, json=req)
    ...
    def _retrieve_single(self, search_query: str, num_ret: int):
        ...
        for rd in search_server_resp:
            url = rd.get('url', '')
            title = rd.get('title', '')
            retrieved_scores.append(float(rd.get('score', '')))
    ...
    def retrieve(
        ...
        r= [self._retrieve_single(q, num_ret) for q in queries]
        try:
            retrieved_docs=[g[0] for g in r ]
            retrieved_scoresr=[g[1] for g in r ]
            return retrieved_docs,retrieved_scoresr
        except:
            print()
        return [],[]