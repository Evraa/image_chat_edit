...
class BlenderBot2RagModel(RagModel):
    ...
    def __init__(self, opt: Opt, dictionary: DictionaryAgent, retriever_shared=None):
        ...
        print (f"Has memory encoder {query_encoder}")
    ...
    def has_query_generator(self) -> bool:
        ...
        """
        print(f"query generator existance = {bool(self.query_generator.agents)}")
    ...
    def has_memory_decoder(self) -> bool:
        ...
        """
        print(f"memory decoder existance = {bool(self.memory_decoder.agents)}")
    ...
    def encoder(
        ...
        expanded_input_text = [self.dict.vec2txt(self.query_generator.clean_input(i)) for i in expanded_input]
        print (f"""
        Exact Encoder Input: 
        {expanded_input_text}
        """)
    ...
    def retrieve_and_concat(
        ...
        print(f"""Docs are now fully empty with slots = {int(n_input)} """)
        # 1a. retrieve from faiss or search
        search_indices = self.get_retrieval_indices(
            retrieval_type, RetrievalType.SEARCH
        )
        # print("="*20)
        # print(f"""Docs results of search (e.g. kendra) search Doc counts = {len(search_indices)} :""")
        ...
        # print("="*20)
        # print(f"""Docs results of Memory accessing, Memory Doc counts = {len(memory_indices)} :""")
        ...
        if memory_indices.numel() > 0:
            ...
            history = [self.query_generator.agent_dict.vec2txt(self.query_generator.clean_input(i)) for i  in query_generator_vec]
            x=self.opt["search_server"].split("/")
            retrieve_memory_server= "http://"+x[0]+"/retrieve_memory"
            print(f"retrieve_memory_server= {retrieve_memory_server}")
            import requests
            memory_server_response= requests.post(url=retrieve_memory_server, json={"memories" :generated_memories,"history":history})
            memories_list=memory_server_response.json()["memories"]
            memories=[[Document("",m_i,"") for m_i in memories_list]]
            memory_scores=torch.LongTensor([memory_server_response.json()["scores"]])
        ...
        # print("="*20)
        # print(f"""Docs After adding dummy docs to just be consistant, dummy Doc counts = {len(no_search_indices)} :""")
        ...
        if no_search_indices.numel() > 0:
            ...
            # print(f"""{dummy_docs}""")
        print("="*20)
        print(f"""All Docs counts = {len(top_docs)} :""")
        print(f"""{top_docs}""")
        print(f"""All Docs scours:""")
        print(f"""{doc_scores}""")
...
class LongTermMemory(RagRetriever):
    ...
    def score_memories(self, query_enc: torch.Tensor) -> List[torch.Tensor]:
        ...
        Score memories.
        """
        print (f"""Class: LongTermMemory:
        funciton: score_memories
        query_enc: 
        {query_enc}
        self.active_memory_slots:
        {self.active_memory_slots} """)
        scores = []
