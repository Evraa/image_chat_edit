...
class QueryGenerator(BB2SubmoduleMixin):
    ...
    def classify_retrieval(
        ...
        # pre_text = [self.agent_dict.vec2txt(self.clean_input(i)) for i in input]
        # texts=[("\n"+pre.split("\n")[-1])*2+"\n".join(pre.split("\n")[-3:])+("\n"+pre.split("\n")[-1])*2 for pre in pre_text ]
        # texts = ["\n".join(self.agent_dict.vec2txt(self.clean_input(i)).split("\n")[-3:]) for i in input]
        pre_text = [self.agent_dict.vec2txt(self.clean_input(i)) for i in input]
        texts=[]
        flags=[]
        for pre in pre_text:
            if any([w in pre.split("\n")[-1].split(" ") for w in ["my","your"]]):
                texts.append(pre.split("\n")[-1])
                flags.append(1)
                continue
            flags.append(0)
            if len(pre.split("\n")[-1].split(" "))<3:
        ...
        for i, s in enumerate(search_queries):
            if (
                ...
                if flags[i]:
                    searches.append(s)
                    continue
                search_modified=texts[i].split(sep="\n")[-1]
                for s_i in s_list:
                    if s_i.lower() not in search_modified.lower():
                        search_modified+= " "+ s_i
                searches.append(search_modified)
        print (f"""
            knowledge_access_method: {self.knowledge_access_method}
            retrieval_type pre query:  {self.retrieval_type}
            given the fact that: NONE = 1, SEARCH = 2, MEMORY = 3
            search_queries:
            {search_queries}
            searches: 
            {searches} 
...
class MemoryDecoder(BB2SubmoduleMixin):
    ...
    def generate_memories(
        ...
        questions= ["what","where","how","when","why","which","who","whose"]
        ...
        for idx, i in enumerate(input):
            ...
            for context_line in context_lines:
                counter_index+=1
                for q in questions:
                    index= context_line.lower().find(q)
                    if index != -1:
                        try:
                            index_end = index+min([context_line[index:].find(stop) for stop in [".","?",",",":"] if context_line[index:].find(stop) != -1])
                            context_line=context_line[:index]+ context_line[index_end+1:]
                            context_line=context_line.strip()
                        except:
                            context_line=context_line[0:index].strip()
                
                if context_line !="" and ( (context_line.lower()[:2] =="i " ) or (" i " in context_line.lower()) or(" my," in context_line.lower()) or  (context_line.lower()[:2] =="my " ) or (" my " in context_line.lower()) or (" mine" in context_line.lower()) or (" me." in context_line.lower())or (" me " in context_line.lower()) ):
                    if counter_index%2!=0:
                        user_context_lines.append(context_line)
                    else:
                        model_context_lines.append(context_line)

            raw_memories_i_user = list(reversed(self._batch_generate(user_context_lines)))
            raw_memories_i_model = list(reversed(self._batch_generate(model_context_lines)))
            memories_i_user=[f'partner\'s persona: {raw_memories_user}'for raw_memories_user in raw_memories_i_user if raw_memories_user != NOPERSONA]
            memories_i_model=[f'your persona: {raw_memories_model}'for raw_memories_model in raw_memories_i_model if raw_memories_model != NOPERSONA]
            memories_i=memories_i_model+memories_i_user
